...
class DeepSeekAdapter(BaseAdapter):
    ...
    def _to_openai_messages(self, messages: list[Message]) -> list[dict[str, Any]]:
        """Convert Message list to OpenAI format (DeepSeek is OpenAI-compatible)."""
        result = []
        for msg in messages:
    ...
    async def probe_capabilities(self) -> ModelCapabilities:
        """Probe DeepSeek capabilities."""
        model_key = self._cfg.model.lower()
        limit = DEFAULT_CONTEXT_LIMITS.get(model_key, DEFAULT_CONTEXT_LIMITS["default"])
        if "deepseek-reasoner" in model_key:
            limit = 65536
        elif "deepseek-chat" in model_key:
            limit = 65536
        elif "deepseek-coder" in model_key:
            limit = 16384
