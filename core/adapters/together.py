...
DEFAULT_CONTEXT_LIMITS = {
    ...
    "meta-llama/Llama-3.1-8B-Instruct-Turbo": 131072,
...
class TogetherAdapter(BaseAdapter):
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        if not config.get("api_key"):
            raise ProviderConfigError("Together adapter requires api_key")
        self._cfg = TogetherConfig(
    ...
    async def chat(
        ...
        try:
            ...
            logger.error(f"Together AI API error: {e.response.status_code} - {e.response.text}")
            raise
    ...
    async def probe_capabilities(self) -> ModelCapabilities:
        """Probe Together AI capabilities."""
        model_key = self._cfg.model
