...
class SambaNovaConfig:
    """Configuration for SambaNova provider."""
...
class SambaNovaAdapter(BaseAdapter):
    ...
    async def chat(
        ...
        """Send chat request to SambaNova API."""
        ...
        try:
            ...
            logger.error(f"SambaNova API error: {e.response.status_code} - {e.response.text}")
            raise
