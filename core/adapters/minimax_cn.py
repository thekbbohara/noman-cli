"""MiniMax China adapter (different endpoint)."""
...
@dataclass
class MiniMaxCNConfig:
    """Configuration for MiniMax CN provider."""
...
class MiniMaxCNAdapter(BaseAdapter):
    """Adapter for MiniMax API (China endpoint)."""

    def __init__(self, config: dict[str, Any]) -> None:
        super().__init__(config)
        if not config.get("api_key"):
            raise ProviderConfigError("MiniMax CN adapter requires api_key")
        self._cfg = MiniMaxCNConfig(
            api_key=config["api_key"],
            model=config.get("model", "abab6.5"),
    ...
    async def chat(
        ...
        """Send chat request to MiniMax CN API."""
        ...
        try:
            ...
            logger.error(f"MiniMax CN API error: {e.response.status_code} - {e.response.text}")
            raise
