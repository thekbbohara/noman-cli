...
class HuggingFaceAdapter(BaseAdapter):
    ...
    async def _get_client(self) -> httpx.AsyncClient:
        if self._client is None:
            if not self._cfg.base_url:
                raise ProviderConfigError("HuggingFace adapter requires base_url for the inference endpoint")
    ...
    async def probe_capabilities(self) -> ModelCapabilities:
        """Probe HuggingFace capabilities."""
        max_ctx = self._cfg.max_context_tokens or DEFAULT_CONTEXT_LIMITS["default"]
        return ModelCapabilities(
            model_name=self._cfg.model or "huggingface",
            max_context_tokens=max_ctx,
            max_output_tokens=self._cfg.max_output_tokens,
            supports_tool_calling=False,  # HF Inference API may not support tool calling
