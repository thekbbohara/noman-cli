...
class DashScopeConfig:
    """Configuration for Alibaba DashScope provider."""
...
class DashScopeAdapter(BaseAdapter):
    ...
    async def probe_capabilities(self) -> ModelCapabilities:
        """Probe DashScope capabilities."""
        model_key = self._cfg.model.lower()
        limit = DEFAULT_CONTEXT_LIMITS.get(model_key, DEFAULT_CONTEXT_LIMITS["default"])
        if "qwen-max" in model_key:
            limit = 32768
        elif "qwen-plus" in model_key:
            limit = 131072
        elif "qwen-turbo" in model_key:
            limit = 8192
        elif "qwen-long" in model_key:
            limit = 1000000
